...
pub fn apply_config_map_with_mode(
    ...
    for (key, value) in map {
        match key.as_str() {
            ...
            "bt-require-crypto" | "bt-min-crypto-level" if mode == ConfigParseMode::Strict => {
                anyhow::bail!(
                    "legacy BitTorrent crypto config key '{}' is not supported",
                    key
                );
            }
...
"#;
    ...
    #[test]
    fn strict_mode_rejects_legacy_bt_crypto_keys() {
        for key in ["bt-require-crypto", "bt-min-crypto-level"] {
            let mut config = GlobalConfig::default();
            let mut map = HashMap::new();
            map.insert(key.into(), "true".into());
