...
use crate::{
    ...
    session::{CheckedIncomingConnection, PieceSelectionStrategy},
...
fn sort_candidate_pieces_by_strategy<F>(
    strategy: PieceSelectionStrategy,
    candidates: &mut [ValidPieceIndex],
    mut availability_of: F,
) where
    F: FnMut(ValidPieceIndex) -> usize,
{
    if matches!(strategy, PieceSelectionStrategy::RarestFirst) {
        candidates.sort_by_key(|piece| availability_of(*piece));
    }
}
...
impl PeerHandler {
    ...
    fn reserve_next_needed_piece(&self) -> anyhow::Result<Option<ValidPieceIndex>> {
        let Some((i_am_choked, bitfield)) = self
            .state
            .peers
            .with_live(self.addr, |live| (self.locked.read().i_am_choked, live.bitfield.clone()))
        else {
            return Ok(None);
        };
        ...
        let mut candidates = {
            let g = self.state.lock_read("reserve_next_needed_piece_candidates");
        ...
        sort_candidate_pieces_by_strategy(
            self.state.shared.options.piece_selection_strategy,
            &mut candidates,
            |piece| self.state.peers.piece_availability(piece),
        );
...
#[cfg(test)]
mod tests {
    use super::{sort_candidate_pieces_by_strategy, PieceSelectionStrategy};
    use librqbit_core::lengths::Lengths;

    #[test]
    fn current_piece_strategy_preserves_candidate_order() {
        let lengths = Lengths::new(4, 1).expect("lengths");
        let mut candidates = vec![
            lengths.validate_piece_index(0).expect("piece 0"),
            lengths.validate_piece_index(1).expect("piece 1"),
            lengths.validate_piece_index(2).expect("piece 2"),
        ];

        sort_candidate_pieces_by_strategy(PieceSelectionStrategy::Current, &mut candidates, |_| 0);

        assert_eq!(
            candidates.iter().map(|piece| piece.get()).collect::<Vec<_>>(),
            vec![0, 1, 2]
        );
    }

    #[test]
    fn rarest_first_piece_strategy_prefers_lower_availability() {
        let lengths = Lengths::new(4, 1).expect("lengths");
        let mut candidates = vec![
            lengths.validate_piece_index(0).expect("piece 0"),
            lengths.validate_piece_index(1).expect("piece 1"),
