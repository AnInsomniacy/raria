...
pub struct SessionOptions {
    ...
    /// Override the DHT bootstrap nodes instead of using the default public set.
    pub dht_bootstrap_addrs: Option<Vec<String>>,
    /// Override the UDP listen address used by the DHT node.
        ...
        async move {
            ...
            let dht = if opts.disable_dht {
                ...
                let dht = if opts.disable_dht_persistence
                    || opts.dht_bootstrap_addrs.is_some()
                    || opts.dht_listen_addr.is_some()
                {
                    if !opts.disable_dht_persistence
                        && (opts.dht_bootstrap_addrs.is_some() || opts.dht_listen_addr.is_some())
                    {
                        warn!(
                            "DHT bootstrap/listen overrides are set; skipping persistent DHT restore"
                        );
                    }
                    DhtBuilder::with_config(DhtConfig {
                        cancellation_token: Some(token.child_token()),
                        bootstrap_addrs: opts.dht_bootstrap_addrs.take(),
                        listen_addr: opts.dht_listen_addr.take(),
                        ..Default::default()
                    })
                    .await
