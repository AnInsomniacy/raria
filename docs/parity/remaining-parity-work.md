...
| BitTorrent runtime fidelity | DHT, PEX, fastresume, daemon/RPC peer-detail UX, end-to-end SOCKS5 torrent completion | `wired` | librqbit-backed service, BT dispatch tests, daemon RPC smoke now cover magnet creation + pause/resume, daemon tracker smoke verifies `bt-tracker` announce on the real daemon path, and new `raria-bt` product-path smokes cover real torrent transfer plus SOCKS5 relay feasibility | Add daemon/RPC-facing peer-detail verification plus transport-specific evidence for DHT/PEX/fastresume lanes and proxied torrent completion |
