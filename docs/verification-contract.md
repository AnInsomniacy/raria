...
| Metalink piece-checksum mismatch surfacing through daemon status | daemon-path automated coverage | `crates/raria-cli/tests/session_smoke.rs` |
...
2. A late-stage closure declaration requires a fresh rerun of the full verification matrix. This document cannot substitute for that run.
