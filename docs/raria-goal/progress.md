...
The completion audit found that the library fixtures were not enough because the `raria` binary still exited immediately. The binary now parses CLI arguments, runs direct URI downloads, starts a foreground RPC server when `--enable-rpc` is set, and drives a background download loop for RPC-created tasks. Final smoke verification covered direct CLI HTTP download plus RPC add/poll/background completion.
